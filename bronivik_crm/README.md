...
> 📚 **Общая документация проекта**: [README](../README.md) | [English](../README.en.md)

Telegram-бот для почасового бронирования кабинетов с интеграцией в основной сервис [Bronivik Jr](../bronivik_jr/README.md).

Часть комплексной системы [Medical Booking Service](../README.md) для автоматизации бронирования медицинских ресурсов.
...
- **Автоматические бэкапы**: фоновое резервное копирование базы данных SQLite (для PostgreSQL используйте `pg_dump`/managed-backup)
...
│   ├── db/           # Работа с SQLite/PostgreSQL (кабинеты, брони)
...
Поддерживаются SQLite (WAL для конкурентного доступа) и PostgreSQL.
Для безопасной работы в единой БД с `bronivik_jr` CRM использует префикс таблиц `crm_`.

Основные таблицы (CRM namespace):

- `crm_users` — пользователи Telegram
- `crm_cabinets` — физические кабинеты
- `crm_cabinet_schedules` — расписание работы кабинетов
- `crm_hourly_bookings` — почасовые бронирования
