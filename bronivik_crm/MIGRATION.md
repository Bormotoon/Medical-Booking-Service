...
### ⚠️ Требует внимания:
- Запуск `go mod tidy` для обновления зависимостей (требуется Go 1.22+)
...
CRM бот подключается к API основного сервиса для проверки доступности оборудования. Синхронизация является опциональной и настраиваемой:

1. В конфиге CRM (`configs/config.yaml` или `.env`) можно включить/выключить синхронизацию через `api.enabled`.
2. В случае недоступности Bronivik Jr API, бот продолжит работу, но выдаст пользователю предупреждение о том, что расписание может быть неактуальным.
3. API ключи настраиваются через `CRM_API_KEY` и `CRM_API_EXTRA`.
