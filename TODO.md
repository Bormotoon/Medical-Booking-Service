...
### 0.2 База данных
- [ ] **Спроектировать схему БД для обоих ботов**
...
- [ ] **Реализовать планировщик напоминаний**
...
- [ ] **Добавить inline-кнопку в напоминание**
...
- [ ] **Реализовать UI для вкл/выкл напоминаний**
...
- [ ] **Реализовать задачу экспорта**
  - Описание: 1-го числа каждого месяца формировать Excel-файл со всеми таблицами БД.
  - Рекомендации:
    - Библиотеки: `openpyxl` (Python), `excelize` (Go).
    - Каждая таблица — отдельный лист.
    - Имя файла: `{Месяц}_{Год}.xls` (например, `Январь_2026.xls`).
    - Месяц — на русском языке.

#### 1.2.2 Отправка отчёта менеджеру
- [ ] **Отправить файл в чат менеджера**
  - Описание: После генерации отправить документ через Telegram Bot API.
  - Рекомендации:
    - Получить `chat_id` менеджера из таблицы `managers`.
    - Использовать `sendDocument`.

#### 1.2.3 Автоудаление старых заявок
- [ ] **Реализовать очистку данных старше 31 дня**
  - Описание: После экспорта (или отдельной задачей) удалять записи.
  - Рекомендации:
    - SQL: `DELETE FROM bookings WHERE created_at < NOW() - INTERVAL '31 days'`.
    - Логировать количество удалённых записей.
    - Опционально: архивировать в отдельную таблицу перед удалением.
